...
class Performer:
    def __init__(self):
        ...
        # weight_decay为权值衰退，主要作用是解决过拟合
        self.optimizer = torch.optim.Adamax(lr=opt.learn_rate, params=params, weight_decay=opt.l2reg)
    ...
    def prepare_data(self):
        ...
        self.embedding_matrix = Vectorizer(self.vocabulary, opt.embedding_dim). \
            generate_embeddingMatrix(embedding_fileName=opt.embedding_file_path,
                                     dat_fname='{0}_{1}_embedding_matrix.dat'.format(str(opt.embedding_dim),
                                                                                     opt.dataset))
        # 2_构建 Dataset
        # 1) 得到所有数据
    ...
    def _train(self):
        ...
        for i_epoch in range(opt.epoch):
            ...
            logger.info('> val_acc: {:.4f}, val_f1_score: {:.4f}'.format(val_acc, val_f1))
            # 保留最大的val_acc
    ...
    def run(self):
        ...
        test_acc, test_f1 = self._evaluate_acc_f1(self.test_dataLoader)
        logger.info('>> test_acc: {:.4f}, test_f1_score: {:.4f}'.format(test_acc, test_f1))
